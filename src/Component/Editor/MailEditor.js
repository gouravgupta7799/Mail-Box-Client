...
const MailEditor = () => {
  ...
  const sendHandler = async (e) => {
    e.preventDefault();
    const res = await fetch(`https://mailboxclient-dc189-default-rtdb.firebaseio.com/${userData.localId}/mailSent.json`, {
      method: 'POST',
      body: JSON.stringify({
        sentTo: toSender,
        subject: subject,
    ...
    if (res.ok) {
      ...
      const secRess = await fetch(`https://mailboxclient-dc189-default-rtdb.firebaseio.com/${userData.localId}/mailRecived.json`, {
        method: 'POST',
        body: JSON.stringify({
          sentTo: toSender,
          subject: subject,
      ...
      console.log('<<', secRess)
      if (secRess.ok) {
        const secData = await secRess.json()
      } else {
        return secRess.json().then((data) => window.alert(data.error.message))
      }
    } else {
