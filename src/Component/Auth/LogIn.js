...
export default function Login() {
  ...
  async function submitHandler(e) {
    e.preventDefault();
    if (!toggle) {
      const res = await fetch('https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDPb8_eVDcDttXa2z1AZsFefzvseA_1Ba4', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        console.log(data)
        dispatch(userAction.localIdUpdater(data.localId))
        dispatch(userAction.tokenUpdater(data.idToken))
      ...
      const res = await fetch('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDPb8_eVDcDttXa2z1AZsFefzvseA_1Ba4', {
        method: 'POST',
